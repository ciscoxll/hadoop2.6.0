...
public class IPAccess extends Configured implements Tool {
	...
	public static void main(String[] args) throws Exception {
		ToolRunner.run(new IPAccess(), args);
	}
	public static class IPAccessMapper extends Mapper<LongWritable, Text, IPTime, NullWritable>{
		Text k2 = new Text();
		LongWritable v2 = new LongWritable();
		
		@Override
		protected void map(LongWritable key, Text value, Mapper<LongWritable, Text, IPTime, NullWritable>.Context context)
	...
	public static class IPAccessReducer extends Reducer<Text, LongWritable, Text, Text>{
		...
		protected void reduce(Text k2, Iterable<LongWritable> v2s,
			...
			TreeSet<LongWritable> treeSet=new TreeSet<>();
			for (LongWritable longWritable : v2s) {
				treeSet.add(longWritable);
			}
			System.out.println(k2+":"+treeSet);
			int count=0;
			long minTime=0;
			for (LongWritable currentTime : treeSet) {
				if (++count==1) 
					minTime=currentTime.get();
				long time = currentTime.get()-minTime;
				System.out.println("currentTime.get()-minTime="+time);
				System.out.println("count="+count);
				if(time>=30*60*1000){
					v3.set(formatDate(minTime)+"\t"+formatDate(currentTime.get())+"\t"+time);
					minTime=currentTime.get();
					context.write(k2, v3);
				}
			}
	...
	public int run(String[] args) throws Exception {
		...
		job.setJarByClass(IPAccess.class);
